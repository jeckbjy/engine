...
D3D12_Texture::D3D12_Texture(ID3D12Device* device, const TextureDesc& desc)
	: Texture(desc)
	, m_handle(NULL)
{
	D3D12_RESOURCE_DESC dx_desc;
	dx_desc.Dimension = D3D12_Mapping::getTextureType(desc.type);
	dx_desc.Alignment = 0;
	...
	dx_desc.Format = D3D12_Mapping::getFormat(desc.format);
...
void* D3D12_Texture::map(PixelData& data, MAP_FLAG flag, uint level /* = 0 */, uint face /* = 0 */)
{
	return 0;
}
