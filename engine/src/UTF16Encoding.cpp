...
const char* UTF16Encoding::m_names[] =
{
	"UTF-16",
	"UTF16",
	NULL
...
const TextEncoding::CharacterMap UTF16Encoding::m_charMap =
{
...
int UTF16Encoding::convert(int ch, unsigned char* bytes, int length) const
{
	if (ch <= 0xFFFF)
	{
		if (bytes && length >= 2)
		{
			uint16_t ch1 = m_flipBytes ? ByteOrder::flip((uint16_t)ch) : (uint16_t)ch;
	...
	else
	{
		if (bytes && length >= 4)
		{
			...
			if (m_flipBytes)
			{
				w1 = ByteOrder::flip(w1);
				w2 = ByteOrder::flip(w2);
			}
